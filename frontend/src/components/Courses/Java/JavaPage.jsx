...
const JavaPage = () => {
  ...
  const fetchCourseData = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/courses/java`, {
        ...
        {modules.map((module, moduleIndex) => {
          ...
          return (
            <React.Fragment key={module.id}>
              <section className={`java-theme-path ${themeClass}`}>
                <div className="java-theme-header">
                  <h2 className="java-path-title">
                    <span className="java-theme-index">THEME {module.order}</span>
                    <span className="java-theme-name">{module.title?.toUpperCase()}</span>
                  </h2>
                </div>
                <div className="java-path-column">
                  {module.nodes.map((node) => (
                    <button
                      key={node.id}
                      type="button"
                      className={["java-path-node", node.type, node.status].join(" ")}
